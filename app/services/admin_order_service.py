...
def order_summary_to_dict(db: Session, order: Order) -> dict[str, Any]:
    return {
        "id": order.id,
...
def list_admin_orders(
    ...
    return {
        "items": [order_summary_to_dict(db, order) for order in orders],
        "pagination": {
            "page": page,
            "limit": safe_limit,
            "total": total,
...
def assign_driver_manually(db: Session, user: User, order_id: int, payload: AdminAssignDriver) -> dict[str, Any] | JSONResponse:
    ...
    old_status = order.status
    active_bids = list(db.scalars(select(Bid).where(Bid.order_id == order.id, Bid.status == "active").with_for_update()))
...
def close_active_bids(db: Session, order_id: int) -> None:
    bids = list(db.scalars(select(Bid).where(Bid.order_id == order_id, Bid.status == "active").with_for_update()))
    for bid in bids:
        bid.status = "closed"
        db.add(bid)
