...
export type UiActiveOrder = {
  id: number; code: string; from: string; to: string; status: string;
  gross: number; commission: number; net: number; offeredPrice: number;
...
export function mapActiveOrder(o: Record<string, unknown>): UiActiveOrder {
  ...
  // The price the client is offering (shown on feed orders before a winner /
  // final price exists).
  const offeredPrice = toNumber((o.client_price ?? o.suggested_price ?? o.final_price) as never);
  return {
    id: Number(o.id),
    ...
    gross, commission: fee, net, offeredPrice,
