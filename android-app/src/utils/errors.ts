...
export function getUzbekErrorMessage(error: unknown): string {
  ...
  if (error instanceof Error && error.message) {
    // A failed fetch (server unreachable / wrong URL / backend down) throws a
    // TypeError "Network request failed" — surface that as a connectivity issue
    // rather than the generic error so the cause is obvious.
    if (error instanceof TypeError || /network request failed|failed to fetch|network/i.test(error.message)) {
      return "Serverga ulanib bo'lmadi. Internet va manzilni tekshiring";
    }
